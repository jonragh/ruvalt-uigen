...
export function HeaderActions({ user, projectId }: HeaderActionsProps) {
  ...
  const handleExportJsx = () => {
    ...
    // Prefer /App.jsx, otherwise take the first root-level file
    let exportPath = "/App.jsx";
    let content = files.get("/App.jsx");

    if (!content) {
      const rootEntry = Array.from(files.entries())
        .filter(([p]) => p.split("/").filter(Boolean).length === 1)
        .sort(([a], [b]) => a.localeCompare(b))[0];

      if (!rootEntry) return;
      [exportPath, content] = rootEntry;
    }

    const filename =
      exportPath.split("/").pop()?.replace(/\.(tsx?)$/, ".jsx") ??
      "component.jsx";
